...
export class IndexerParameterUpdate {
  ...
  constructor(indexerEntity: IndexerEntity, currentBlock: ethereum.Block) {
    ...
    let updateId = this.indexerEntity.id.concat('-').concat(currentBlock.number.toString())
    this.indexerParameterUpdateEntity = new IndexerParameterUpdateEntity(updateId)
    this.indexerParameterUpdateEntity.updatedAtTimestamp = currentBlock.timestamp
  ...
  registerUpdate(newIndexingRewardCutRatio: BigDecimal, newQueryFeeCutRatio: BigDecimal): void {
    let updated = true
    // Don't store the update for the very first one
    updated = updated && !(
      (this.indexerParameterUpdateEntity.previousIndexingRewardCutRatio == null) &&
      (this.indexerParameterUpdateEntity.previousQueryFeeCutRatio == null)
    )

    // Don't store the update if there is no change
    updated = updated && (
      this.indexerParameterUpdateEntity.previousIndexingRewardCutRatio.notEqual(newIndexingRewardCutRatio) || 
      this.indexerParameterUpdateEntity.previousQueryFeeCutRatio.notEqual(newQueryFeeCutRatio)
    )
